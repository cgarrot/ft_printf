...
/*   Updated: 2019/04/17 18:15:51 by seanseau    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
{
	if ((dec != 0 && !(flags.flag == 'p') && !(flags.width > ft_strlen(c_hexa->str))) || (flags.flag == 'p' && flags.minus))
	{
		ft_puthash(flags);
		c_hexa->surplus = 2;
	}
...
int		check_p_w_hexa(unsigned long long dec, t_flags flags)
{
	...
	{
		if (flags.hashtag && flags.point)
			if ((tmp = is_hexa_width(dec, flags, &c_hexa) != -1))
				return (tmp);
		if (flags.hashtag && flags.point && dec == 0)
		{
			if (!(flags.flag == 'p'))
