...
/*   Updated: 2019/03/28 16:25:58 by cgarrot     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int		check_p_w_digit(long long digit, t_flags flags)
{
	...
	if (((flags.space && (digit >= 0) && !flags.plus)) && (((digit == 0 && flags.space) || 1 < ft_strlen(number)) || flags.precision > ft_strlen(number)) && flags.flag != 'u')
		ft_putchar(' ');
	else if (flags.space)
	{
		flags.space = 0;
	...
	if (flags.width && !flags.precision)
	{
		...
		if (flags.width > ft_strlen(str))
		{
			...
			else
			{
				...
				{
					if ((flags.width >= ft_strlen(str) + 1) && flags.hashtag && surplus != 1)
						ft_putncaract(' ', size - 1);
					else
						ft_putncaract(' ', size - surplus);
				}
		...
		else
		{
			...
			if (ft_strlen(str) > 6 && flags._h == 1)
				ft_putncaract(' ', flags.width - 6);
			if (ft_strlen(str) > 3 && flags._h == 2)
...
int		check_p_w_hexa(long long dec, t_flags flags)
{
	int		size;
	char	*str;
	int		surplus;
	...
	while (str[++compt.i])
	{
		...
		if (str[compt.i] == '%' && (str[compt.i + 1] != '%' && str[compt.i + 1] != '\0'))
		{
			...
			while (!(ft_isflags(str[compt.j++])))
			{
				...
				if (str[compt.j] == '0' && flags.zero < 1 && !ft_isflags(str[compt.j + 1]) && !ft_isdigit(str[compt.j - 1]))
					flags.zero++;
				if (str[compt.j] == '.' && ((ft_isdigit(str[compt.j - 1]) || ft_isdigit(str[compt.j + 1])) || str[compt.j - 1] == ' ' || str[compt.j - 1] == '+' || (str[compt.j - 1] == '%' && ft_isflags(str[compt.j + 1]))))
				{
					flags.point++;
					compt.k = compt.j;
				}
